...
# OPEN_AI DALLE 3
# =========================
def generate_dalle3_image(prompt): 
    """
    Calls a MidJourney wrapper API to generate an image. 
    """
    print("Generating DALL-E 3 image for:", prompt)
...
def generate_images(dataset):
    ...
    for item in dataset:
        try: 
            ...
            image_files = []
            img_im= generate_dalle3_image(item["prompt_im"])
            ...
            img_ex = generate_dalle3_image(item["prompt_ex"])
            ...
            print("Prompt Errored out on: " + item["id"])
            print(f"Error: {e}")
            continue
...
def generate_html(index_data):
    ...
    """
    html_path = os.path.join(OUTPUT_FOLDER, "index_wording.html")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dalle3-api-key", type=str, default=None)
    args = parser.parse_args()
    #Load OpenAI client
    client = OpenAI(api_key=args.dalle3_api_key)
