...
#hugging face - litellm tester code (image input)



def benchmark_model(type, model_name, benchmark_dir, device = "cpu"):
    if type == "OPEN-AI":
    ...
    #if type == "HUGGING_FACE": #need to figure out how to change the code to work for uploading images to hugging face datasets
        #os.environ["HF_TOKEN"] = "..."
        #messages = [
            #{
                #"role": "user",
...
#LlaMa Models - Vision 3.2, LlaMa 4 + Deepseek(hugging face)
models_providers = []
models_org_user = [] 
models_modelname = []
combined_completion = []
for i in range(len(models_providers)):
    combined_completion.append(models_providers[i] + models_org_user[i] + models_modelname[i])
print(combined_completion)

results_huggingface = {f'huggingface-{model}': benchmark_model(model, args.directory) for model in combined_completion }
#
